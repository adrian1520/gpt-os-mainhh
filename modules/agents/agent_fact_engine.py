...
def normalize(statement):
    ...
    # 🔥 procedural core (z Twoich danych)
    if "prawomocno" in s:
        return "final_decision", "positive"

    if "brak doręczenia" in s or "nie doręczono" in s:
        return "delivery_issue", "negative"

    # 🔥 kontakty / child relations
    if "odmawia" in s or "nie chce spotkań" in s:
        return "contact_refusal", "negative"

    if "spokojnie" in s or "dobrze" in s or "chętnie" in s:
        return "contact_positive", "positive"

    # 🔥 rozszerzenie: konflikty narracji
